...
void queueInit	(Queue* q){
	...
	q->back = q->Anchor;
	q->iterator = q->Anchor;
	q->posReturned = -1;	//-1 indicated no node was found
}
...
Node* findValue (Queue* q, char value){
	...
	if (isFull(q) == -1) return NULL;
	//else iterate through all the nodes until a
...
void InsertAfter(Queue* q, char value){
	//By inserting after a node, the newly inserted node will be
	//Pulled right after the node that the new node in inserted after

	//If the Queue is empty, this operation will
	//have the same effect as pushing to an empty queue
	if(isFull(q) == -1) {
		push(q, value);
	}
	//if the iterator points to the back of the queue,
	//This operation have the same effect as the push
	else if(q->iterator == q->back){
		push(q, value);
	}
	else{
		Node* node = (Node*) malloc(sizeof(Node));
		node->data = value;
		node->next = q->iterator->next;
		node->prev = q->iterator;
		q->iterator->next->prev = node;
		q->iterator->next = node;
	}
...
void deleteCurrent(Queue* q){
	...
	//Free the node that iterator is pointing to.
	//Move the iterator toward the back, if thats not
	//possible, then move the iterator to the front
	else{
		if(q->iterator == q->front){
			Node temp = *(q->iterator);
			free(q->iterator);
			q->front = temp.next;
			q->iterator = q->front;
		}
